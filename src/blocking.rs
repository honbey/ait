...
/// Run a blocking operation (DB IO) on a dedicated blocking thread so it does
/// not stall the tokio async executor.
///
...
/// on a blocking thread. Holding the Mutex on a tokio worker thread stalls
/// every other task on that thread (including in-flight proxy streams).
///
